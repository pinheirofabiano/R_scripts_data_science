...
df_final <- remove_nonexp(df, method = "median", min_exp = 0) 

# filter low expressed genes
df_final <- filter_by_variance(df_final, n = 1000)
?filter_by_variance
...
pancreatitis_module_brown <- which(gcn$genes_and_modules$Modules == "brown")

pancreatitis_module_brown_final <- gcn$genes_and_modules[pancreatitis_module_brown,]
...
if (websiteLive) {
  enriched <- enrichr(pancreatitis_module_brown_final$Genes, dbs)
}
